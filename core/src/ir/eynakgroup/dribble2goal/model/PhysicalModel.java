...
//            System.out.println("D2G log ::: " + "start rendering *********************");
            startRendering();
        }

        if (matchStat.GAME_STATE == Constants.GAME_WAIT_FOR_AFTER) {
            if (matchStat.isOppDataRecieved) {
//                System.out.println("D2G log ::: " + "After recieved -------------------------");
        ...
        } else if (matchStat.GAME_STATE == Constants.GAME_WAIT_FOR_AFTER_PENALTY) {
            if (matchStat.isOppDataRecieved) {
                PhysicalModel.this.screen.endofRound();
            }
...
//            System.out.println("D2G log ::: " + "sending after ..........." + data);
    ...
    private void gotoStartPositionFake() {
        matchStat.GAME_STATE = Constants.GAME_WAIT_FOR_AFTER;
    ...
    private void gotoStartPosition() {
        ...
        for (int i = 0; i < 5; i++) {
            bodyOppPlayers[i].fix();
            bodyOppPlayers[i].setPosition(new Vector2(Constants.SCREEN_WIDTH, Constants.SCREEN_HEIGHT));
        }
        for (int i = 0; i < 5; i++) {
            bodyMyPlayers[i].fix();
...
//                            System.out.println("wait for after penlaty *****************************");
                            sendReady();
                        }
                    }
                });
